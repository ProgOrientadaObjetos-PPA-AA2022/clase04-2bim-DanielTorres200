    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < nombres.length; i++) {
            ...
            if (edad[i] >=0 && edad[i] <= 18) {
                ...
                pasajeMenor.establecerPersona(persona);
                pasajes.add(pasajeMenor);
                
            }else if (edad[i] > 18 && edad[i] <= 25) {
                PasajeUniversitario pasajeUniversitario = new PasajeUniversitario(pasajeFijo);
                pasajeUniversitario.establecerPersona(persona);
                pasajes.add(pasajeUniversitario);
                
            }else if (edad[i] > 25 && edad[i] < 65) {
                PasajeNormal pasajeNormal = new PasajeNormal(pasajeFijo);
                pasajeNormal.establecerPersona(persona);
                pasajes.add(pasajeNormal);
                
            }else if (edad[i] >= 65) {
                PasajeTerceraEdad pasajeMayorEdad = new PasajeTerceraEdad(pasajeFijo);
                pasajeMayorEdad.establecerPersona(persona);
                pasajes.add(pasajeMayorEdad);
            }
        }
